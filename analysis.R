...
##Import Aurin json
library(rjson)
aurin_data = fromJSON(file = "Output.json")
##Import Tweet .csv
library(utils)
Tweet_data = read.csv(file = "TwitterSentiment.csv", header = TRUE)
...
#jpeg("correlation matrix.jpeg", width = 550, height = 550)
#chart.Correlation(as.data.frame(list(Not_obese, Not_overweight, Physically_active, Not_smokers)), method = c("pearson"), pch="+", histogram = TRUE)
#dev.off()
...
Negative_sentiment = Tweet_data$NEG_Count/(Tweet_data$POS_Count+Tweet_data$Neu_Count+Tweet_data$NEG_Count)
Net_positive = (Tweet_data$POS_Count - Tweet_data$NEG_Count)/(Tweet_data$POS_Count+Tweet_data$Neu_Count+Tweet_data$NEG_Count)

jpeg("barplot of counts.jpeg", width = 1000, height =1000)
barplot(t(matrix(c(Tweet_data$POS_Count, Tweet_data$Neu_Count, Tweet_data$NEG_Count), nr=10)), names.arg = Tweet_data$City, beside = TRUE)
dev.off()
...
jpeg("Positive_vs_Obese.jpeg", width = 550, height = 550)
...
jpeg("Positive_vs_Overweight.jpeg", width = 550, height = 550)
par(fig=c(0,0.8,0,0.8), new=TRUE)
...
jpeg("Positive_vs_PhysicalActivity.jpeg", width = 550, height = 550)
par(fig=c(0,0.8,0,0.8), new=TRUE)
...
jpeg("Positive_vs_Smoker.jpeg", width = 550, height = 550)
par(fig=c(0,0.8,0,0.8), new=TRUE)
...
jpeg("Importance.jpeg", width = 550, height = 550)
